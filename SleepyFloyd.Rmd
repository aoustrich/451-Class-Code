...
---
DIAGNOSTICS
- `gelman.diag` (Gelman-Rubin diagnostic): measures how close the chains are to the point. We want it to be close to 1. Sometimes called the "R hat" $\hat{R}$
- `raftery.diag` (Raftery-Lewis diagnostic): measures how many samples are needed to get a good estimate? We want it to be small? 
  - Fellingham likes a Dependence factor cutoff of around 3?
...
effectiveSize(sleepy.out$samples)
allsamps <- as.matrix(sleepy.out$samples) # adds all samples to a matrix
dim(allsamps) 
